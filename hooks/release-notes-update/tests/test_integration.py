...
def test_retires_fragment_when_branch_has_no_cur_commits(tmp_path, monkeypatch):
    """If a branch's tracked fragment is stale relative to the current
    `[CUR-XXX]` set on the branch — e.g. after a rebase / commit-message
    amend that dropped every `[CUR-XXX]` subject — the hook must retire
    the fragment rather than leave it behind.

    Simulates the post-amend state: a commit that carries the prior
    hook's fragment file but whose subject no longer has a `[CUR-XXX]`
    prefix (so `commit_subjects_since_main()` finds nothing that
    `fragment_from_commits` keeps)."""
    repo = tmp_path / "consumer"
    _init_repo(repo)
    _commit(repo, "VERSION", "v1.0.0+1", "initial")
    subprocess.check_call(["git", "-C", str(repo), "branch", "origin/main"])
    subprocess.check_call(["git", "-C", str(repo), "checkout", "-q", "-b", "feat/qux"])
    # Write a fragment + a code change, and commit both under a subject
    # WITHOUT a [CUR-XXX] prefix — modelling the post-amend state where
    # the dev dropped the ticket reference but the fragment file
    # generated by the prior hook run is still in the tree.
    frag_dir = repo / ".release-notes"
    frag_dir.mkdir()
    frag_file = frag_dir / "feat-qux.md"
    frag_file.write_text(
        "<!-- release-notes-fragment v1 -->\n"
        "<!-- version: v1.0.0+1 -->\n"
        "\n"
        "- [CUR-99] stale entry from a dropped commit\n"
    )
    (repo / "code.txt").write_text("hello")
    subprocess.check_call(["git", "-C", str(repo), "add", "."])
    subprocess.check_call(["git", "-C", str(repo), "commit", "-q", "-m", "wip refactor without ticket prefix"])

    monkeypatch.chdir(repo)
    rc = main(["--version-command", "cat VERSION"])
    assert rc == 0
    assert not frag_file.exists(), "stale fragment should have been retired"

...
- **`@<sha>` vs `@main` for consumers**: documented in `README.md`. Consumers pin to `@main` for auto-update or `@<sha>`/`@<tag>` for stability. Within this repo's own wrappers we use `./.github/actions/<name>` (path) so dogfood checks always run against the PR's version, not against `main`.
- **Tag releases per action**: tags are subdir-prefixed (`slack-notify-vX.Y.Z`), not repo-wide `vX`, because the repo hosts multiple independent actions.
...
- **Branch protection is not Terraformed**: would create a bootstrap cycle with `hht_admin` (which consumes from this repo). The rules live in `docs/branch-protection.md` and are applied via `gh api`; update both that doc and the protection rules together when adding a new required check.
...
- `docs/branch-protection.md` — exact `gh api` invocation to apply protection, plus why review enforcement is intentionally deferred during the bootstrap phase.
...
- `hooks/` — Cure-HHT-specific pre-commit hooks shared to consumer repos. Currently contains `release-notes-update/`; reserved for future hooks (e.g. `elspais`-driven requirement-traceability checks) that aren't appropriate for a public hooks repo.

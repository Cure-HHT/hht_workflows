...
- **PR titles**: must contain a Linear issue ref in brackets (`[CUR-1296]`, `[TOOL-90]` — any team) or `[Dependabot]`. Squash-merges use the PR title verbatim as the `main` commit subject, so this guards downstream traceability. The `validate-pr-title` action enforces it.

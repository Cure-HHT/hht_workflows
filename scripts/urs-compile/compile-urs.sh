...
#   6. Writes docs/<name>-build-provenance.md naming the revision of every
#      out-of-repo input the deliverables were compiled from: the pinned
#      commit where a source was obtained from its published artifact, and
#      git's description where it was a working tree. That record is what
#      ties a committed deliverable to the source it enumerates.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Identity lookups for the provenance record. A tree reaching this compile is
# either an obtained artifact carrying stamps or an operator's git checkout, and
# only one of those answers `git describe`.
# shellcheck source=scripts/urs-compile/source-identity.sh
...
\`${NAME}-term-index.pdf\`, \`${NAME}-term-index.docx\`) are compiled from sources this
repository does not hold: the compile pipeline itself, and every repository
whose requirements federate into the document. This file records which revision
of each was read.

Where a source was obtained from the artifact its repository published for a
pinned commit, the row below names that commit. Where it was a working tree, the
row names git's description of it.
...
EOF
  ...
  if [ -n "${ASSOCIATE_ROOT:-}" ]; then
    ASSOC_SLUG="$(source_slug "${ASSOCIATE_ROOT}")"
    ASSOC_VERSION="$(source_version "${ASSOCIATE_ROOT}")"

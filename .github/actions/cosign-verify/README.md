...
Wraps `sigstore/cosign-installer@v3` and performs keyless signature verification
using the Cure-HHT standard certificate-identity-regexp and OIDC issuer. Implements
a retry/backoff loop (the Sigstore verify side can flake) and an optional soft-fail
mode that mirrors the SBOM-attestation pattern.
